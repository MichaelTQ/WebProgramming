...
if(isset($_POST['uname']) && isset($_POST['email']) && isset($_POST['tel'])
    && isset($_POST['straddr']) && isset($_POST['city']) && isset($_POST['state']) 
    && isset($_POST['zcode']))
{
    ...
    $sql_update_cus = 'update normal_user set phone = \''.$tel.'\', st_address = \''.$straddr.'\', city = \''.$city.'\', state = \''.$state.'\', zip_code = \''.$zcode.'\' where id = \''.$id.'\';';
    $sql_result1 = mysqli_query($link, $sql_update_user) or die('Query failed!'.mysql_error());
    
    $_SESSION['uname'] = $uname;
    
    $sql_result2 = mysqli_query($link, $sql_update_cus) or die('Query failed!'.mysql_error());
